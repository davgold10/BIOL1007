...
# Function: dfmaker
# makes a dataframe
# input: m1 (mean 1), m2 (mean 2)
# outputs: data frame

set.seed(123)
dfmaker <- function(m1, m2) {
...
data <- dfmaker(10, 45)
head(data)
# not particularly reproducible, as only the means are manipulated. Could do nearly the same, but add sd and n to the function's objects to allow for more manipulation
```
...
```{r}
test <- aov(value~type, data = dfmaker(10, 45))
summary(test)
```
...
```{r}
# Function: hm
# find the p value of a dataframe with 2 columns
# input: df (two column data frame)
# outputs: p value from ANOVA chart

thisPval <- function(df=NULL) {
  aovtest <- aov(value~type, data = df)
  # pvalue <- summary(aovtest)[[1]][["Pr(>F)"]][1] #did function in one less line, but pulls only from aovtest from one event. should redo an aov test, and look like the following
